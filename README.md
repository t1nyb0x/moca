...
**3D キャラクターを机の上に置いて、話しかけられるようにする Windows アプリ。**

VRM のモデルを読み込み、LLM と会話する。返答に応じて表情が変わり、口が動き、
体が動く。枠を消して机の上へ置けば、ほかの作業をしながら隣に居させられる。

- **会話** — ローカル LLM（Ollama / LM Studio / llama.cpp）でも、外部の API
  （OpenAI / Anthropic / Google Gemini）でも繋がる。接続先は会話を続けたまま
  切り替えられる
- **表情** — 返答に混ぜられた感情タグを読み、喜び・悲しみ・驚きなどを表情と姿勢に
  移す。タグは本文からは取り除かれるので、画面には出ない
- **声** — VOICEVOX と CeVIO AI で読み上げる。**声に合わせて口が動く。** 感情に
  応じて声色も変わる
- **身振り** — 手を振るなどの動きを、返答のタグに合わせて再生する。手を振る動き
  を一つ同梱している
- **机に置く** — 枠と背景を消し、モデルだけが立つ。描かれていないところは背後の
  ウィンドウを操作できる。掴んで動かせる
- **待機中も止まらない** — まばたき、視線、呼吸、体重の移動。話していないときも
  人形には見えない

モデルは同梱していない。**利用者が自分で用意した VRM を読み込む。**
...
初回は次の順に進む。**接続先とキャラクターを作るまで、モデルは開けない。**
どのキャラクターにモデルを結び付けるかが決まらないためである。
...
モデルが無くても会話はできる。**顔を出さずに使うこともできる**（「3D を隠す」）。
...
同梱するのは**自分たちで作った動きだけ**とする。他人が作ったファイルは、
...
**API キーは Windows の資格情報マネージャーへ預ける**ので、上のファイルには
入らない。ログにも出ない。
