...
現在の版は 0.7.0。VRM / PMX の表示、LLM との会話、感情に応じた表情と姿勢の
変化、音声での読み上げと音声駆動リップシンク、接続先の切り替え、デスクトップ
マスコット表示まで動作する。PMX は表示だけの実験的対応と位置づけている。
...
返答に合わせて体を動かせる。**動きは同梱していない。** VRMA ファイルを用意して
割り当てる。

1. 設定の「キャラクター」→「身振り」で「VRMA を追加」を押す
...
動かないときは診断パネルを開く。割り当てごとに読み込めたかどうかが出て、
その場で試せる。直近の応答にタグが何度出たかも分かるので、「モデルがタグを
出していない」のか「ファイルを読めていない」のかを切り分けられる。
