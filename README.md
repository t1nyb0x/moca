...
キャラクターの 3D モデルは同梱していません。お手元の VRM を読み込んでお使いください。
...
表情は、LLM の返答に混ざった `[happy]` のようなタグで変わります。タグは moca が
読み取って取り除きますので、画面には出ません。

声は VOICEVOX と CeVIO AI に対応しています。感情で声色も変わります。手を振るなどの
身振りも、同じタグで動かせます。動きはひとつだけ同梱してあります。
...
| （任意）VRMA ファイル | 身振りの動きが入ったファイルです。同梱以外の動きをさせる場合に使います |
...
初回は次の順に進みます。接続先とキャラクターを作るまで、VRM は開けません。どの
キャラクターに結び付けるかが決まらないためです。
...
「接続を確かめる」を押すと、その接続先で使えるモデルの一覧を取り込みます。そこから
選んでください。
...
ウィンドウを見失ったときは、タスクトレイのアイコンから呼び戻せます。
...
**入れるのはホストとポートまでです。** その先の経路は moca が付けます。`/v1` まで
書くと `/v1/v1/chat/completions` になって繋がりません。末尾のスラッシュは、
付いていても構いません。

「接続を確かめる」を押すと `/v1/models` を叩いて、使えるモデルの一覧を取り込みます。
候補から選べば、モデル名を手で書かずに済みます。

LM Studio は Developer タブからサーバーを起動しておいてください。ポートを既定から
変えている場合は、それに合わせます。

### 音声で読み上げる

声を作るアプリ（VOICEVOX や CeVIO AI）は、moca とは別に動きます。先に起動して
おいてください。
...
設定の「キャラクター」から、使うアプリと待ち受け先を選びます。「接続を確かめる」を
押すと話者を取り込みます。

shirataki の待ち受け先を既定から変えている場合は、ここも合わせてください。

CeVIO には 2 通りの繋ぎ方があります。「直接」は同じ PC の CeVIO AI をそのまま呼び
ますので、ほかに何も起動しなくて済みます。そのかわり、別の PC の CeVIO は使えま
せん。声だけ別の PC に任せたい場合は、shirataki 経由をお選びください
（[ADR-0018](docs/adr/0018-cevio-over-com.md)）。

「感情の割り当てを作る」を押すと、その話者が持っている感情の名前から、組み合わせを
推測します。持っている感情はキャストごとに違います。当てはまるものが無い感情は、
声色を変えず、抑揚と速さだけで差を出します。値は手でも調整できます。
...
同梱してありますが、ボタンを押すまでは使われません。何も足さなければ、返答は
これまでどおりです。
...
同梱の `wave` は、自分たちで撮影した動きを切り出したものです。作り方は
[`src-tauri/resources/gestures/README.md`](src-tauri/resources/gestures/README.md)
にあります。使ったコマンドもそのまま載せてあります。

手持ちの VRMA を整えるにも、同じ道具が使えます。
...
診断には、割り当てごとに読み込めたかどうかが出ます。直近の返答にタグが何度出たかも
分かります。「LLM がタグを出していない」のか「ファイルを読めていない」のかを
切り分けられます。

| 制限 | 理由 |
|---|---|
| VRM のみ | VRMA は VRM の骨組み（ボーン）の名前を前提としています。PMX は名前が決まっていません |
| タグ名は英小文字のみ | 感情タグと同じ文法で拾うためです。外れた名前は本文として画面に出ます |
| 感情タグと同じ名前は使えない | 感情として先に解決されます |
| 表情と視線は動かない | 読み上げ中の口や、感情の表情を上書きしないためです |
| その場から動かない | 机に置いているとき、ウィンドウは VRM の形に合わせて詰めてあります。位置が変わると外へ出ます |
...
まず「診断」を開いてください。読み込んだ VRM の中身が分かります。骨組みの数、
使える表情の名前、身振りが載ったかどうか。表情と身振りを手で試すボタンと、ログの
保存先もここにあります。

| 症状 | 見るところ |
|---|---|
| 返答が来ない | 接続先の「接続を確かめる」。ローカルなら本体が起動しているか |
| 表情が変わらない | 診断でタグの回数を見ます。0 なら LLM がタグを出していません |
| 声が出ない | 合成器を起動しているか。設定の「接続を確かめる」で話者を取り込めるか |
| 身振りが動かない | 診断で読み込めているか。VRM か。タグ名は英小文字か |
| 表示が重い | 「3D を隠す」をお試しください。VRM が大きい場合は警告が出ます |
...
- 身振りで手が届く先は、VRM ごとにずれます。腕が低く垂れる VRM では、手も低くなります
...
CI は PR でだけ回る。取り込む前に確かめれば足りるし、push でも回すと同じ中身を
二度検査することになる。版上げを push した時点で 1 回、その commit で PR を開いて
1 回、`main` へ取り込んでもう 1 回、という具合だった。

`main` は Release ワークフローも守る。タグを打つ前に型チェックとテストを回し、
通らなければインストーラを作らない。
