...
4. `happy` が立っている間は `blink` をその重みぶん薄める（**暫定対応**）

規則 4 は暫定である。`happy` の表情は目もとを細める形を含むことが多く、そこへ
`blink` を重ねると笑顔のまま目だけが開閉して落ち着かない。恒久策は表情側の
目もとと `blink` を別の系統として扱うことで、それが入ったらこの規則は外す。
判定は正規化のあとに行う。画面に実際に出る `happy` の強さで薄めたいため。
